...
from hopes.ope.estimators import SelfNormalizedPerDecisionImportanceSampling
...
def wpdis_daily(
    ...
    """Estimate daily self-normalized per-decision IS with optional stickiness correction. This
    wrapper adapts notebook-style flat arrays of taken propensities to the estimator API.
    ...
    :param num_days: Number of episodes / days.
    :param steps_per_episode: Number of timesteps per episode.
    :param rew_flat: Flat reward array of shape `(N,)`.
    :param act_flat: Logged action indices of shape `(N,)`.
    :param p_b_taken_flat: Behavior policy probabilities on logged actions, shape `(N,)`.
    :param p_e_taken_flat: Target policy probabilities on logged actions, shape `(N,)`.
    :param sticky_act_flat: Sticky action indicator, shape `(N,)`.
    :param eps: Numerical stabilizer.
    :param clip: Optional symmetric clipping threshold for importance ratios.
    :param num_bootstrap_samples: Number of bootstrap samples used for the confidence interval.
    :param significance_level: Significance level used for the confidence interval.
    :param normalization: Normalization method for WIS. Can be "global" or "per_timestep".
    :returns: Tuple of (mean, lower_bound, upper_bound) for the estimated policy value.
    """
    ...
    def build_probs_from_taken(p_taken: np.ndarray) -> np.ndarray:
        ...
        :param p_taken: Shape (N,), probabilities of taken actions.
        :returns: Shape (N, A), valid probability matrix for all actions.
        """
